...
SPACE = hp.choice('network_layout', [
    {
        ...
        'dense_layers': [scope.int(hp.quniform('nbr_filters', 10, 16, 1))] *
                         scope.int(hp.quniform('nbr_layers', 3, 5, 1)),
        'batch_size': 64,

        # Params
        'growth_rate': scope.int(hp.quniform('growth_rate', 12, 30, 6)),
        'start_lr': 10 ** scope.int(hp.quniform('start_lr', -4, -1, 1)),
        'dropout': hp.quniform('dropout', 0, 0.2, 0.05),
	'compression': hp.quniform('compression', 0.4, 0.6, 0.05),
    }
])
